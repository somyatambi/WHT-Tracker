...
const DEFAULT_HABITS: Habit[] = [
  { id: 'default-habit-1', name: 'Wake by 6am', order: 0, createdAt: new Date().toISOString() },
  { id: 'default-habit-2', name: 'Exercise', order: 1, createdAt: new Date().toISOString() },
  { id: 'default-habit-3', name: 'Read 20 pages', order: 2, createdAt: new Date().toISOString() }
];
...
export function useHabits() {
  ...
  const addHabit = (name: string) => {
    setHabits(prev => [...prev, { id: uuidv4(), name, order: prev.length, createdAt: new Date().toISOString() }]);
  };
